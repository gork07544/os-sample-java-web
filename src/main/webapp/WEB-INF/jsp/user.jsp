...
<title> User info TVSConsult| Business consulting | Aalborg </title>
 <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="description" content="See information about user account information">
...
<c:choose>
  <c:when test="${pageContext.request.userPrincipal.authenticated}">
	...
	<li class="active"><a href="${userUrl}"><span class="glyphicon glyphicon-user"></span>
...
<div class="container-fluid" style="padding:0px;margin-top:-20px;"> 
 ...
 <c:url var="attachuserURL" value="/attachuser"/>
<form action="${attachuserURL}" method="post">       
	...
	<br />
	<fieldset class="form-group fildsetCust">
  		<label for="authority"><h4 style="color: #1d1c1a;"  class="fildsetH4">Authority</h4></label>
  		<select id="authority" name="authority" class="form-control">
   			<option value="ROLE_USER">ROLE_USER</option>
   			<option value="ROLE_ADMIN">ROLE_ADMIN</option>
   			<option value="ROLE_STAF">ROLE_STAF</option>
   			<option value="ROLE_COOL">ROLE_COOL</option>
		</select>
	</fieldset>
	<input type="hidden"                        
		...
		value="${_csrf.token}"/>
	<div>
		<button type="submit" class="btn btnfr btn-lg" style="height:60px;"><h4 style="line-height:20px;font-weight: bold;font-size: 16px; ">Change authority</h4></button>
	</div>
	<br />
		...
		User name: <c:out value="${message.username}" />
		</h4>
		<h4 style="color: #1d1c1a;text-align: left;">
		Password: <c:out value="${message.password}" />
		</h4>
		<h4 style="color: #1d1c1a;text-align: left;">
		Enabled: <c:out value="${message.enabled}" />
		</h4>
	
	User authorities: 
	<ul>
 		<c:forEach items="${message.authorities}" var="authority">
	...
	User companies: 
	<ul>
 		<c:forEach items="${message.companys}" var="company">
        <li><c:out value="${company.name}" /></li>
 		<li><c:out value="${company.address}" /></li>
 		
 		<li>Company applications
        	<ul>
        	 	<c:forEach items="${company.applications}" var="application">
            		<c:url var="contactUrl" value="/applications?applicationId=${application.id}"/>
            		<li><a href="${contactUrl}"><c:out value="${application.applicationName}" /></a></li>
            		<li><c:out value="${application.user.username}" /></li>
 				</c:forEach>
        	</ul>
    	</li>
 		</c:forEach>
